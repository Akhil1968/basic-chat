...
var httpHandler = function(req, res){
	fs.readFile(__dirname + '/index3.html', function(error, fileData){
	if (error){
...
io.on('connection', function(socket){  // step 3: Handle  standard event 'connection' on io object and receive a client socket
  ...
  socket.on('chat-message', function(msg, callback){ // step 5: write handler for various user events 
    console.log('message: ' + msg);
	callback("callback " + msg);
    io.emit('chat-message', 'server broadcast ' + msg);  // step 6: use emit method on io to broadcast ao all connected users
  });
});
